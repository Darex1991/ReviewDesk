...
export class FileStorageService {
  ...
  async listByEntityRef(entityRef: string): Promise<StoredFile[]> {
    return this.db
      .select()
      .from(file)
      .where(and(eq(file.entityRef, entityRef), isNull(file.deletedAt)))
      .orderBy(file.createdAt);
  }

  async deleteByEntityRef(entityRef: string): Promise<void> {
    const files = await this.listByEntityRef(entityRef);

    await Promise.all(
      files.map((stored) => this.deleteFile(stored.storageKey)),
    );
  }

  generateEntityRef(entityType: "user" | "review", entityId: string) {
